...
export default function NewPostPage() {
  ...
  const handleSubmit = async (published: boolean) => {
    if (!form.title || !form.content) {
      setError('Title dan konten wajib diisi')
      return
    }
    ...
    router.push(published ? `/@${data.author?.username ?? ''}/${data.slug}` : '/dashboard/posts')
    router.refresh()
  }
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
      <div style={{ marginBottom: '1.75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#ffffff' }}>New Post</h1>
      </div>
          ...
          {(['markdown', 'html'] as const).map((t) => (
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: `1px solid ${form.type === t ? '#0070f3' : '#222222'}`,
                background: form.type === t ? '#0a1929' : '#111111',
                color: form.type === t ? '#0070f3' : '#888888',
                fontSize: '0.875rem',
                fontWeight: 500,
                cursor: 'pointer',
              }}
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input
          ...
          style={inputStyle}
          placeholder="Ringkasan singkat (tampil di feed)"
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
        <div>
          ...
          <input
            ...
            style={inputStyle}
            placeholder="Tech, Tutorial, Finance..."
          />
        </div>
        <div>
